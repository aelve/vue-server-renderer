(function (global, factory) {
  ...
  async function renderNode (node, isRoot, context) {
    if (node.isString) {
      renderStringNode$1(node, context);
    } else if (isDef(node.componentOptions)) {
      await renderComponent(node, isRoot, context);
    } else if (isDef(node.tag)) {
      renderElement(node, isRoot, context);
    } else if (isTrue(node.isComment)) {
      if (isDef(node.asyncFactory)) {
        // async component
        await renderAsyncComponent(node, isRoot, context);
      } else {
  ...
  async function renderComponent (node, isRoot, context) {
    ...
    if (isDef(getKey) && isDef(cache) && isDef(name)) {
      ...
      var get = context.get;
      if (isDef(has)) {
        has(key, async function (hit) {
          if (hit === true && isDef(get)) {
            get(key, async function (res) {
            ...
            await renderComponentWithCache(node, isRoot, key, context);
          }
        });
      } else if (isDef(get)) {
        get(key, async function (res) {
      ...
      await renderComponentInner(node, isRoot, context);
    }
  }

  async function renderComponentWithCache (node, isRoot, key, context) {
    var write = context.write;
    write.caching = true;
  ...
  async function renderComponentInner (node, isRoot, context) {
    ...
    await renderNode(childNode, isRoot, context);
  }

  async function renderAsyncComponent (node, isRoot, context) {
    var factory = node.asyncFactory;

    var resolve = async function (comp) {
      ...
      if (resolvedNode) {
        if (resolvedNode.componentOptions) {
          // normal component
          await renderComponent(resolvedNode, isRoot, context);
        } else if (!Array.isArray(resolvedNode)) {
          // single return node from functional component
          await renderNode(resolvedNode, isRoot, context);
        } else {
    ...
    if (factory.resolved) {
      await resolve(factory.resolved);
      return
    }
